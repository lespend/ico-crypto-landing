...
NioApp = function (NioApp, $, window, document) {
    ...
    NioApp.Plugins.currencyWidget = function() {
        const widgets = document.querySelectorAll('.currency-widget__item')
        widgets.forEach(widget => {
            ...
            script.innerHTML = JSON.stringify({
                "symbol": widget.dataset.widgetSymbol,
                "width": "100%",
                "height": "100%",
                "locale": "ru",
                "dateRange": "3M",
    ...
    NioApp.components.docReady.push(NioApp.Plugins.currencyWidget);
    ...
    NioApp.Plugins.converter = async function() {
        ...
        let toSelect = document.querySelector('.converter__field-group--default .select');
        const swapButton = document.querySelector('.converter__swap-button');
        let lastValidValue = '';
        let currentRate = await getCurrentRate();

        async function getCurrentRate() {
            const url = 'https://api.coinpaprika.com/v1/price-converter';
            const params = new URLSearchParams({
                base_currency_id: fromSelect.value,
                quote_currency_id: toSelect.value,
                amount: 1,
            })
        ...
        const convertValue = (value) => {
            toField.value = currentRate * value;
        }
        ...
        const swapButtonHandler = async () => {
            // Добавляем или удаляем класс чтобы поменять инпуты местами в css
            converter.classList.toggle('converter--swaped')

            // Убираем обработчик ввода с прошлого инпута и добавляем новому
            fromField.removeEventListener('input', changeHandler);
            toField.addEventListener('input', changeHandler);

            // Меняем инпуты местами
            [fromField, toField] = [toField, fromField];
            [fromSelect, toSelect] = [toSelect, fromSelect];

            // Прошлый блокируем новый разблакируем
            fromField.disabled = false;
            toField.disabled = true;

            // Обновляем текущий курс валют
            currentRate = await getCurrentRate()
        }

        fromField.addEventListener('input', changeHandler)

        $('.select').on('select2:select', async function(e) {
            currentRate = await getCurrentRate()
            convertValue(fromField.value);
        });
